...
public class EasyIK : MonoBehaviour
{
    ...
    public void SolveIK(Transform target)
    {
        ...
        for (int i = 0; i < jointPositions.Length - 1; i += 1)
        {
            ...
            Transform parentTransform = jointTransforms[i].transform.parent;
            robotController.listTargetJointsPosition[i] = parentTransform.InverseTransformPoint(jointPositions[i]);
            ...
            robotController.listTargetJointsRotation[i] = targetRotation * startRotation[i];
        }
